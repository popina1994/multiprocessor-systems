...
int main(int argc, char *argv[])
{
	...
	MPI_Request rcvRequest2;
	MPI_Request rcvRequest3;
	...
	while (epsilon <= diff)
	{
		...
		*/
		if (MASTER_RANK != rank)
		{
			...
			MPI_Isend(&wPar[0][curBeginCol], 1, columnType, rank - 1, 2, MPI_COMM_WORLD, &sendRequest2);
			//printf("Poslata : %lf od %d kolona %d  tag 2\n", wPar[1][curBeginCol], rank, curBeginCol);
		}

		if ((NUM_PROCESSES - 1) != rank) {
			MPI_Isend(&wPar[0][curEndCol], 1, columnType, rank + 1, 3, MPI_COMM_WORLD, &sendRequest3);
		...
		if (MASTER_RANK != rank)
		{
			...
			MPI_Irecv(&wPar[0][curBeginCol - 1], 1, columnType, rank - 1, 3, MPI_COMM_WORLD, &rcvRequest3);
		...
		if ((NUM_PROCESSES - 1) != rank)
		{
			...
			MPI_Irecv(&wPar[0][curEndCol + 1], 1, columnType, rank + 1, 2, MPI_COMM_WORLD, &rcvRequest2);
		...
		*/
		...
		for (i = 1; i < M - 1; i++)
		{
			for (j = curBeginCol + 1; j <= curEndCol - 1; j++)
			{
		...
		if ((NUM_PROCESSES - 1) != rank) {
			MPI_Wait(&sendRequest3, &status);
		}

		if (MASTER_RANK != rank) {
			MPI_Wait(&rcvRequest3, &status);
		}
		if ((NUM_PROCESSES - 1) != rank) {
			MPI_Wait(&rcvRequest2, &status);
		}
		for (row = 0; row < M; row++)
		{
			for (col = curBeginCol - 1; col <= curBeginCol - 1; col++)
			{
				u[row][col] = wPar[row][col];
			}
		}
		for (i = 1; i < M - 1; i++)
		{
			for (j = curBeginCol; j <= curBeginCol; j++)
			{
				wPar[i][j] = (u[i - 1][j] + u[i + 1][j] + u[i][j - 1] + u[i][j + 1]) / 4.0;
		...
		for (row = 0; row < M; row++)
		{
			for (col = curEndCol + 1; col <= curEndCol + 1; col++)
			{
				u[row][col] = wPar[row][col];
			}
		}
