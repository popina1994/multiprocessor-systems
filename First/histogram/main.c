...
int main(int argc, char* argv[]) {
  ...
  unsigned int idx;
  unsigned int i;
  unsigned int  glMax = 0;
  unsigned int  glMin = UINT_MAX;
  pb_SwitchToSubTimer(&timers, parStr,  pb_TimerID_COMPUTE);

  // Divide.
  //
  #pragma omp parallel for num_threads(MAX_THREADS) private(idx) reduction(min : glMin) \
          reduction(max : glMax)
  for (idx = 0; idx < img_height * img_width; idx ++)
  {
  ...
  unsigned sizeHisto = glMax - glMin + 1;
  ...
  for (iter = 0; iter < numIterations; iter++)
  {
    ...
    memset(histoThread, 0, sizeHisto * MAX_THREADS * sizeof(unsigned char));

    #pragma omp parallel num_threads(MAX_THREADS)
    {
      unsigned int threadId;
      unsigned int i;
      unsigned int len = (img_width * img_height) / MAX_THREADS;
      unsigned int sizeImg = img_height * img_width;
      #pragma omp for private(threadId, i)
      for (threadId = 0; threadId < MAX_THREADS; threadId ++)
      {
        ...
        */
        ...
        {
          //printf("Thread id :%d\n", omp_get_thread_num());
          for (i = threadId * len; i <  (threadId + 1) * len;  ++i)
          //for (i = threadId * len; i < ((MAX_THREADS == threadId + 1) ? (sizeImg) : (threadId + 1) * len);  ++i)
          {
            const unsigned int value = img[i];
      ...
      for (i = 0; i < sizeHisto; i ++)
      {
        ...
        if (total > UINT8_MAX)
        {
          histoPar[glMin + i] = UINT8_MAX;
        }
        else
        {
          histoPar[glMin + i] = (unsigned char)total;
        }
      }

    }
    //printf("Kraj svega bitnog/nebitnog\n");
  }
  ...
  int testPassed = 1;
  for (idx = 0; idx <= histo_width * histo_height; idx ++)
  {
    if (histoPar[idx] != histoSeq[idx])
    {
      testPassed = 0;
      printf("Razlikuju se na %d par :  %d histoSeq %d \n", idx, histoPar[idx], histoSeq[idx]);
    }
  }
